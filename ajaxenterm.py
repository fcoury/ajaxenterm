...
class Multiplex:
	def __init__(self,cmd=None,domname=None):
	...
	def create(self,w=80,h=25):
		...
		if pid==0:
			...
			if self.cmd:
				...
				#cmd=['/bin/login']
				#cmd=['/usr/bin/telnet', 'localhost', '9002']
				cmd=['/bin/ls']
			else:
			...
			# Get Xen pty
                        if (self.domname):
                            domid=int(getDomIDFromName(self.domname))
                        
			    if (domid != -1):
		                xenpty=getTTYByDomID(domid)
                                if (self.xenfd != -1):
                                    print "Closing xen fd"
                                    os.close(self.xenfd)
                                    self.xenfd=-1
                                if (self.xenfd == -1):
                                    print "Opening xen fd"
                                    fd=os.open(xenpty,os.O_RDWR+os.O_NONBLOCK)
                                    self.xenfd=fd
...
class AjaxTerm:
	def __init__(self,cmd=None,index_file='ajaxterm.html',domname=None):
		self.files={}
		...
		self.multi = Multiplex(cmd,domname)
		self.session = {}
